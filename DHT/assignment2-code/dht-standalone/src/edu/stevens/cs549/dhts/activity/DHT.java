...
//		routing.joinCheck();
	...
	public void join(String uri) throws Failed, Invalid {
		 ...
		 */
		this.state.clear();
		NodeInfo next=this.client.getInfo(uri);
		try {
			succ=this.client.findSuccessor(new URI(uri), info.id);
			this.setSucc(succ);
			this.stabilize();
